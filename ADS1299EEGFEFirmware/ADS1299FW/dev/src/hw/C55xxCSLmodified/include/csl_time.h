...
extern _DATA_ACCESS RTC_TZ _RTC_tz;
...
_CODE_ACCESS time_t     RTC_mktime(struct RTC_tm *_tptr);
_CODE_ACCESS double     RTC_difftime(time_t _time1, time_t _time0);
      _IDECL char      *RTC_ctime(const time_t *_timer);
_CODE_ACCESS char      *RTC_asctime(const struct RTC_tm *_timeptr);
_CODE_ACCESS struct RTC_tm *RTC_gmtime(const time_t *_timer);
_CODE_ACCESS struct RTC_tm *RTC_localtime(const time_t *_timer);
_CODE_ACCESS size_t     RTC_strftime(char *_out, size_t _maxsize, 
				 const char *_format,
				 const struct RTC_tm *_timeptr);
